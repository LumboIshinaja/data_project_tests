...
def test_transaction_id_uniqueness(sales_df):
    """Test that 'transaction_id' is unique in the sales data."""
    duplicate_count = count_duplicates(sales_df, "transaction_id")
    assert duplicate_count == 0, f"Found {duplicate_count} duplicate transaction_id(s)"


@pytest.mark.integrity
def test_required_fields_no_null_values(sales_df):
...
@pytest.mark.integrity
@pytest.mark.business_logic
def test_total_calculation(sales_df):
    """Test that 'total' column is correctly calculated as (price * quantity - discount)."""
    incorrect_total_count = count_incorrect_totals(sales_df)

    assert incorrect_total_count == 0, f"Found {incorrect_total_count} rows with incorrect 'total' values."

@pytest.mark.integrity
def test_id_exists_in_customers(sales_df, customers_df):
    """Test if every customer_id in sales data exists in customers data (foreign key check)."""
    sales_customer_ids = get_distinct_values(sales_df, 'customer_id')
    customers_ids = get_distinct_values(customers_df, 'customer_id')
